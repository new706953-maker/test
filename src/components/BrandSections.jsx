                ...
                {brand.logo_url && brand.logo_url.trim() !== '' ? (
                  <img
                    ...
                    onError={(e) => {
                      ...
                      const heading = e.target.nextElementSibling;
                      if (heading) {
                        heading.classList.remove('sr-only');
                        heading.classList.add('text-3xl', 'font-bold', 'text-secondary', 'mb-4');
                      }
                    }}
                  />
                ) : null}
                <h2 className={brand.logo_url && brand.logo_url.trim() !== '' ? 'sr-only' : 'text-3xl font-bold text-secondary mb-4'}>{brand.name}</h2>
                {brand.banner_url && brand.banner_url.trim() !== '' ? (
                  <img
                    src={brand.banner_url}
                    alt={`${brand.name} Banner`}
                    className="w-full max-w-5xl h-auto object-cover mx-auto mb-6 rounded-lg shadow-md"
                    loading="lazy"
                    decoding="async"
